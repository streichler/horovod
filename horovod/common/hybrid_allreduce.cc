  ...
  cudaStream_t stream, void* rbuf_h, MPI_Datatype mpi_type, MPI_Op mpi_op, MPI_Comm local_comm, MPI_Comm node_comm, int lrank, int nsize)
{
  ...
  if (nsize > 1)
  {
    ...
    if (lrank == 0 or lrank == 2 or lrank == 4 or lrank == 6)
    {
      ...
      CUDACHECK(cudaMemcpyAsync(rbuf_h, rbuf + (shift * blockcount)*typesize, ((lrank == 6) ? blockcount + count%blockcount : blockcount)*typesize,
                  cudaMemcpyDeviceToHost, stream));
      cudaStreamSynchronize(stream);

      PUSH_RANGE("MPI_Allreduce", 0)
      MPI_Allreduce(MPI_IN_PLACE, rbuf_h, ((lrank == 6) ? blockcount + count%blockcount : blockcount), mpi_type, mpi_op, node_comm);
      POP_RANGE

      CUDACHECK(cudaMemcpyAsync(rbuf + (shift * blockcount)*typesize, rbuf_h, ((lrank == 6) ? blockcount + count%blockcount : blockcount)*typesize,
    ...
    NCCLCHECK(ncclBcast(rbuf + (1 * blockcount)*typesize, blockcount, nccl_type, 2, nccl_local_comm, stream));
    NCCLCHECK(ncclBcast(rbuf + (2 * blockcount)*typesize, blockcount, nccl_type, 4, nccl_local_comm, stream));
    NCCLCHECK(ncclBcast(rbuf + (3 * blockcount)*typesize, blockcount + count%blockcount, nccl_type, 6, nccl_local_comm, stream));
  }
}
